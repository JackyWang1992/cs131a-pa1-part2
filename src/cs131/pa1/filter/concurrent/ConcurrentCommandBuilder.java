...
class ConcurrentCommandBuilder {
	
	static ConcurrentFilter createFiltersFromCommand(String command){
		...
		for (String command1 : commands) {
			ConcurrentFilter filter = constructFilterFromSubCommand(command1.trim());
			if (filter != null) {
				filters.add(filter);
			} else {
				return null;
